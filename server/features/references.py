...
def fetch_vars(m, name):
    return [x._tx_position for x in get_children(lambda x: hasattr(x, 'name') and x.name == name, m)]

def fetch_routines(m, name):
    from_root = [x._tx_position for x in get_children(lambda x: hasattr(x, 'name') and x.name == name, m)]
    from_flow = [x._tx_position for x in get_children(lambda x: x.__class__.__name__ in ['Test', 'Screen'] and x.type.name == name, m)]
...
def resolve_references(model, param_name, uri):
    ...
    refs = []
    for o in occurences:
        refs.append(Location(uri=uri, range=pos_to_range(m._tx_parser.pos_to_linecol(o), param_name.__len__())))
